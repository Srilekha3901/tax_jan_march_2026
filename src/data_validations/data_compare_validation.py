...
def data_compare(source_df, target_df, key_columns, num_records=10):
    ...
    smt = source_df.exceptAll(target_df).withColumn(
        "datafrom",
        lit("source")
    )

    tms = target_df.exceptAll(source_df).withColumn(
        "datafrom",
        lit("target")
    )

    failed = smt.union(tms).persist()
    ...
    if failed_count > 0:
        ...
        preview_df = (
            ...
            .withColumn(
                "EffectiveStartDate",
                col("EffectiveStartDate").cast("string")
            )
            .withColumn(
                "EffectiveEndDate",
                col("EffectiveEndDate").cast("string")
            )
        )

        failed_records = preview_df.limit(num_records).collect()
    ...
    if failed_count > 0:
        ...
        failed2 = (
            ...
            .withColumn(
                "hash_key",
                sha2(
                    concat(*[col(c) for c in key_columns]),
                    256
                )
            )
        )
